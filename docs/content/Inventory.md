...
- `Inventory`: factory with `new(category?, maxCapacity?, maxEquipped?, definitions) -> Inventory instance`

## Key Concepts

- Categories: logical namespaces under player inventory (for example: `Default`, `Tools`, `Decorations`).
...
- Transfer: move a single item between two players via `Transfer` (source and destination data are both mutated).
...
- Definitions are server-controlled and stable over time; equipment `maxStack` is coerced to `1`.
...
- Run the unit tests (`Tests/Specs/InventorySpec.lua`) in your Roblox test environment or via your test runner.
- If you want a separate transfer API for cross-game inventories, extend the interface with cross-player synchronization hooks.
