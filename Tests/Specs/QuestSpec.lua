...
describe("QuestService", function()
	...
	describe("StartQuest", function()
		...
		it("rejects starting a quest for nil player", function()
			local ok, err = QuestService.StartQuest(nil, "quest_basic")
			assert.is_false(ok)
			assert.equals("PlayerRequired", err)
		end)
	end)
	...
	describe("ProgressObjective", function()
		...
		it("rejects non-numeric deltas", function()
			QuestService.StartQuest(player, "quest_basic")
			local ok, err = QuestService.ProgressObjective(player, "quest_basic", "collect_coins", "3")
			assert.is_false(ok)
			assert.equals("InvalidDelta", err)
		end)

		it("rejects progress updates for nil player", function()
			local ok, err = QuestService.ProgressObjective(nil, "quest_basic", "collect_coins", 1)
			assert.is_false(ok)
			assert.equals("PlayerRequired", err)
		end)
	...
	describe("ReportItemCollected", function()
		it("increments generic numeric collect objectives", function()
			QuestService.StartQuest(player, "quest_basic")
			local ok, err = QuestService.ReportItemCollected(player, "quest_basic", "any_item", 4)
			assert.is_true(ok)
			assert.is_nil(err)
			local quests = QuestService.GetPlayerQuests(player)
			assert.equals(4, quests["quest_basic"].Progress["collect_coins"])
		end)

		it("returns NoMatchingObjective for non-collect objectives", function()
			QuestService.StartQuest(player, "quest_basic")
			QuestService.ProgressObjective(player, "quest_basic", "collect_coins", 10)
			QuestService.TriggerObjectiveEvent(player, "quest_basic", "visit_npc")
			...
			local ok, err = QuestService.ReportItemCollected(player, "quest_chain_2", "Chest", 1)
			assert.is_false(ok)
			assert.equals("NoMatchingObjective", err)
		end)

		it("rejects invalid quantity values", function()
			QuestService.StartQuest(player, "quest_basic")
			local ok, err = QuestService.ReportItemCollected(player, "quest_basic", "any_item", "bad")
			assert.is_false(ok)
			assert.equals("InvalidQuantity", err)
		end)

		it("rejects collection reports for nil player", function()
			local ok, err = QuestService.ReportItemCollected(nil, "quest_basic", "any_item", 1)
			assert.is_false(ok)
			assert.equals("PlayerRequired", err)
		end)
	end)
